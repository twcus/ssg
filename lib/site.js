...
const primraf = require('./utils/primraf');
...
class Site {
     ...
     */
    constructor(config) {
        const { name, description, url, isProduction } = config;
        Object.assign(this, { name, description, url, isProduction });
        this.config = config;
        this.data = {};
        this.contentMap = {};
        this.pageMap = {};
        ...
        this.contentCache = new NodeCache(); // Caches source filepaths mapped to their rendered content.
        this.pathCache = new NodeCache(); // Caches source filepaths mapped to destination filepaths.
    }
     ...
     */
    get writablePages() {
        return this.allPages.filter(page => !page.isWritten);
    }
    ...
    /**
     * Get an array of the filepaths of assets that need to be compiled, as specified in the config.
     * @returns {string[]} An array of filepaths of the Assets.
     */
    get dynamicAssetPaths() {
        return this.config.assets.compile.map(assetPath => path.join(this.config.assetsDir, assetPath));
    }

    /**
     * Get an array of all the DynamicAssets for this site.
     * @returns {DynamicAsset[]} An array of DynamicAssets.
     */
    get dynamicAssets() {
        return Object.entries(this.assetMap).reduce((acc, [key, value]) => {
            if (this.dynamicAssetPaths.includes(key)) {
                acc.push(value);
            }
            return acc;
        }, []);
    ...
    /**
     * Get all Assets that need to be written.
     * @returns {Object[]} An array of Pages and ContentPages.
     */
    get writableAssets() {
        return this.assets.filter(asset => !asset.isWritten);
    }
     ...
     */
    cleanBuildDir() {
        return primraf(this.config.buildDir);
    }
    ...
    get taskGroups() {
        ...
        if (this.isProduction) {
            // TODO Must add production-specific task(s) to taskGroups (e.g. minify)
        }
        return taskGroups;
    }
