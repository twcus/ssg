...
class Page {
     ...
     */
    get attributes() {
        return Object.assign({}, this.frontmatter);
    }
    ...
    get destPath() {
        ...
        strippedSourceDir = strippedSourceDir.length ? path.join(strippedSourceDir) : [];
    ...
    async read() {
        ...
        this.template = parsedContent.content;
    }

    /**
     * Compile the template and render the HTML content.
     */
    render() {
        const compiledTemplate = this.site.renderer.compile(this.template);
        this.renderedContent = compiledTemplate(this.context);
    }
